...
export default {
    ...
    methods: {
        ...
        fetchItems: function() {
            ...
            database.collection("doctors").where("clinic","==","Ruffles").get().then(querySnapshot => {
            ...
            .then(querySnapshot => {
                ...
                querySnapshot.forEach(doc => {
                    ...
                    if (doctorList.includes(docID)) {
                        ...
                        //console.log(index);
                        let month = doc.data().date.toDate().getMonth(); 
                        // var year = doc.data().date.toDate().getYear(); //have to ensure same year else it accumulates
                        numOfPatients[index][month] += 1;
                        this.datacollection.datasets[index].data[month] = numOfPatients[index][month];
                        ...
                        let m = doc.data().date.toDate().getMonth(); 
                ...
                for (var i=1; i<=12; i++) { //for every month
                    let totalPatients = 0;
                    for (var j=0; j<index;j++) { //for every doctor list
